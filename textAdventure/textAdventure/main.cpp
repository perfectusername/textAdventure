...
int main()
{
		...
		{ // State 2 (pile of ashes)
			{
				// Item 0: (key)
				{
					...
					// Item 1: (the table itself)
					{
						"You smush one part of the pile of ashes into another part of the pile of ashes, but it makes no real difference.",	// Phrase
						42069,											// Item ID
						1											// Use flag
					}
				}
	...
	*/
	...
	list<int>	lockedBuffer{ 1, 2, 0 };
	list<int>	keyIDBuffer{ 1, 0, 0 };

	lookPhrasesBuffer.clear();
	lookPhrasesBuffer = { "It's a normal wooden door.", "It's a normal wooden door, but it's also on fire.", "There is an open arch and a pile of ashes where the door once stood." };
	lookFlagBuffer = { 1 , 1 , 1 };
	list<string>	openPhrasesBuffer{ "You open the unlocked door and step through.", 
						"You cannot open the door. It is locked."};
	...
	// Look at the key
	cout << "You look at the key: " << endl;
	cout << key.getLookPhrase() << endl;
	cout << endl;
